...
var Graph = /** @class */ (function () {
    ...
    Graph.prototype.insert = function (node) {
        ...
        if (isOverwrite) {
            throw new errors_1.NodeAlreadyExistsError(node, this.nodes.get(this.nodeIdentity(node)), this.nodeIdentity(node));
        }
    ...
    Graph.prototype.replace = function (node) {
        var isOverwrite = this.nodes.has(this.nodeIdentity(node));
        if (!isOverwrite) {
            throw new errors_1.NodeDoesntExistError(this.nodeIdentity(node));
        }
        this.nodes.set(this.nodeIdentity(node), node);
    };
    /**
    ...
    Graph.prototype.addEdge = function (node1Identity, node2Identity) {
        ...
        if (!node1Exists) {
            throw new errors_1.NodeDoesntExistError(node1Identity);
        }
        if (!node2Exists) {
            throw new errors_1.NodeDoesntExistError(node2Identity);
        }
     ...
     * @param compareFunc An optional function that indicates the sort order of the returned array
     */
    Graph.prototype.getNode = function (nodeIdentity) {
