...
var require_Grid = __commonJS({
  "node_modules/pathfinding/src/core/Grid.js"(exports, module) {
    ...
    Grid2.prototype.getNodeAt = function(x, y) {
      return this.nodes[y][x];
    };
    Grid2.prototype.isWalkableAt = function(x, y) {
      return this.isInside(x, y) && this.nodes[y][x].walkable;
    };
    Grid2.prototype.isInside = function(x, y) {
      return x >= 0 && x < this.width && (y >= 0 && y < this.height);
    };
    Grid2.prototype.setWalkableAt = function(x, y, walkable) {
    ...
    Grid2.prototype.clone = function() {
      var i, j, width = this.width, height = this.height, thisNodes = this.nodes, newGrid = new Grid2(width, height), newNodes = new Array(height);
      for (i = 0; i < height; ++i) {
...
function createStore(createState) {
  ...
  const subscribeWithSelector = (listener, selector17 = getState, equalityFn = Object.is) => {
    console.warn("[DEPRECATED] Please use `subscribeWithSelector` middleware");
    let currentSlice = selector17(state);
    function listenerToAdd() {
      const nextSlice = selector17(state);
  ...
  const subscribe = (listener, selector17, equalityFn) => {
    if (selector17 || equalityFn) {
      return subscribeWithSelector(listener, selector17, equalityFn);
    }
...
function create(createState) {
  ...
  const useStore2 = (selector17 = api.getState, equalityFn = Object.is) => {
    const [, forceUpdate] = (0, import_react.useReducer)((c) => c + 1, 0);
    const state = api.getState();
    const stateRef = (0, import_react.useRef)(state);
    const selectorRef = (0, import_react.useRef)(selector17);
    ...
    if (currentSliceRef.current === void 0) {
      currentSliceRef.current = selector17(state);
    }
    let newStateSlice;
    let hasNewStateSlice = false;
    if (stateRef.current !== state || selectorRef.current !== selector17 || equalityFnRef.current !== equalityFn || erroredRef.current) {
      newStateSlice = selector17(state);
    ...
    useIsomorphicLayoutEffect(() => {
      ...
      selectorRef.current = selector17;
...
function createContext() {
  ...
  const useStore2 = (selector17, equalityFn = Object.is) => {
    const useProviderStore = (0, import_react2.useContext)(ZustandContext);
    if (!useProviderStore) {
      throw new Error("Seems like you have not used zustand provider as an ancestor.");
    }
    return useProviderStore(selector17, equalityFn);
  };
  const useStoreApi2 = () => {
    const useProviderStore = (0, import_react2.useContext)(ZustandContext);
    if (!useProviderStore) {
...
var getHandleBounds = function getHandleBounds2(selector17, nodeElement, zoom) {
  var handles = nodeElement.querySelectorAll(selector17);
...
var selector$a = function selector5(s) {
  return {
...
var selector$9 = function selector6(s) {
  return {
...
var selector$8 = function selector7(s) {
  return {
    setNodes: s.setNodes,
    setEdges: s.setEdges,
...
var selector$7 = function selector8(s) {
  return {
...
var selector$6 = function selector9(s) {
  return {
    d3Zoom: s.d3Zoom,
    d3Selection: s.d3Selection,
...
var selector$5 = function selector10(s) {
  return {
...
function hasSelector(target, selector17, nodeRef) {
  var current = target;
  do {
    var _current;
    if ((_current = current) !== null && _current !== void 0 && _current.matches(selector17)) return true;
...
var selector$4 = function selector11(s) {
...
var selector$3 = function selector12(s) {
...
var selector$2 = function selector13(s) {
  return {
...
var selector$1 = function selector14(s) {
  return {
...
var round = function round2(x, multiple) {
  if (multiple === void 0) {
    multiple = 10;
  }
  return Math.round(x / multiple) * multiple;
};
var roundDown = function roundDown2(x, multiple) {
  if (multiple === void 0) {
    multiple = 10;
  }
  return Math.floor(x / multiple) * multiple;
};
var roundUp = function roundUp2(x, multiple) {
  if (multiple === void 0) {
    multiple = 10;
  }
  return Math.ceil(x / multiple) * multiple;
};
var toInteger = function toInteger2(value, min) {
  if (min === void 0) {
    min = 0;
  }
...
var pathfindingAStarDiagonal = function pathfindingAStarDiagonal2(grid, start, end) {
  try {
...
var pathfindingAStarNoDiagonal = function pathfindingAStarNoDiagonal2(grid, start, end) {
  try {
    var finder = new import_pathfinding.AStarFinder({
...
var pathfindingJumpPointNoDiagonal = function pathfindingJumpPointNoDiagonal2(grid, start, end) {
  try {
...
var getSmartEdge = function getSmartEdge2(_ref) {
  ...
  try {
    ...
    var _createGrid = createGrid(graphBox, nodeBoxes, source, target, gridRatio), grid = _createGrid.grid, start = _createGrid.start, end = _createGrid.end;
    ...
    var graphPath = smoothedPath.map(function(gridPoint) {
      var x = gridPoint[0], y = gridPoint[1];
    ...
    var index4 = Math.floor(fullPath.length / 2);
    var middlePoint = fullPath[index4];
...
var _excluded3 = ["nodes", "options"];
function SmartEdge(_ref) {
  var nodes = _ref.nodes, options = _ref.options, edgeProps = _objectWithoutPropertiesLoose2(_ref, _excluded3);
  var sourceX = edgeProps.sourceX, sourceY = edgeProps.sourceY, sourcePosition = edgeProps.sourcePosition, targetX = edgeProps.targetX, targetY = edgeProps.targetY, targetPosition = edgeProps.targetPosition, style = edgeProps.style, label = edgeProps.label, labelStyle = edgeProps.labelStyle, labelShowBg = edgeProps.labelShowBg, labelBgStyle = edgeProps.labelBgStyle, labelBgPadding = edgeProps.labelBgPadding, labelBgBorderRadius = edgeProps.labelBgBorderRadius, markerEnd = edgeProps.markerEnd, markerStart = edgeProps.markerStart;
  ...
  if (smartResponse === null) {
    return import_react9.default.createElement(FallbackEdge, _extends({}, edgeProps));
  }
  ...
  if (hasStringLabel) {
    edgeLabel = import_react9.default.createElement(EdgeText$1, {
      x: edgeCenterX,
      y: edgeCenterY,
      label,
      labelStyle,
...
function SmartBezierEdge(props) {
  var nodes = useNodes();
  return import_react9.default.createElement(SmartEdge, _extends({}, props, {
...
//# sourceMappingURL=@tisoap_react-flow-smart-edge.js.map
